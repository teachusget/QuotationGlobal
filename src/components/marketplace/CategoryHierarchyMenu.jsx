...
export default function CategoryHierarchyMenu({ categories, onSelect }) {
  ...
  const slugParams = (categoryName, subcategoryName = '') => { const params = new URLSearchParams({ category: categoryName }); if (subcategoryName) params.set('subcategory', subcategoryName); return params.toString() }
  const choose = (value) => { if (value === 'All') { setOpen(false); navigate('/marketplace/solutions'); return } onSelect(value); setOpen(false) }
  const openCategory = (item) => { setOpen(false); navigate(`/marketplace/type/${type}?${slugParams(item.name)}`) }
  const openSubcategory = (item) => { if (!category) return; setOpen(false); navigate(`/marketplace/type/${type}?${slugParams(category.name, item.name)}`) }
  const chooseType = (value) => { setType(value); window.dispatchEvent(new CustomEvent('marketplace-service-type-selected', { detail: value })) }

  return <div ref={root} className="relative"><button type="button" onClick={() => setOpen(!open)} className="flex items-center gap-1 hover:text-primary">Categories<ChevronDown className={`h-3.5 w-3.5 transition ${open ? 'rotate-180' : ''}`}/></button>{open && <div className="absolute left-0 top-6 z-50 w-[min(760px,calc(100vw-24px))] overflow-hidden rounded-xl border bg-white shadow-overlay"><div className="flex items-center justify-between border-b px-4 py-3"><div><p className="text-sm font-bold text-slate-900">Browse solutions</p><p className="text-[11px] font-normal text-slate-500">Select type, category and subcategory</p></div><button type="button" onClick={() => choose('All')} className="text-[11px] font-semibold text-primary">View all solutions</button></div><div className="grid min-h-64 grid-cols-[190px_1fr_1fr]"><div className="border-r bg-slate-50 p-3">{types.map((item) => <button type="button" key={item.value} onClick={() => chooseType(item.value)} className={`mb-2 flex w-full items-center justify-between rounded-lg px-3 py-2.5 text-left text-xs font-semibold ${type === item.value ? 'bg-primary text-white' : 'bg-white text-slate-700 hover:bg-blue-50 hover:text-primary'}`}>{item.label}<ChevronRight className="h-3.5 w-3.5"/></button>)}</div><div className="border-r p-3"><p className="mb-2 px-2 text-[11px] font-bold uppercase tracking-wide text-slate-400">Categories</p>{availableCategories.map((item) => <button type="button" key={item.id} onMouseEnter={() => setCategoryId(String(item.id))} onFocus={() => setCategoryId(String(item.id))} onClick={() => openCategory(item)} className={`flex w-full items-center gap-2 rounded-lg px-2 py-2 text-left text-xs font-semibold ${category?.id === item.id ? 'bg-blue-50 text-primary' : 'text-slate-700 hover:bg-slate-50'}`}>{item.logo_url ? <img src={item.logo_url} alt="" className="h-7 w-7 rounded object-contain"/> : <span className="grid h-7 w-7 place-items-center rounded bg-slate-100"><ImageIcon className="h-3.5 w-3.5 text-slate-400"/></span>}<span className="min-w-0 flex-1 truncate">{item.name}</span><ChevronRight className="h-3.5 w-3.5"/></button>)}{!availableCategories.length && <p className="p-3 text-xs text-slate-400">No categories available.</p>}</div><div className="p-3"><p className="mb-2 px-2 text-[11px] font-bold uppercase tracking-wide text-slate-400">Subcategories</p>{subcategories.map((item) => <button type="button" key={item.id} onClick={() => openSubcategory(item)} className="flex w-full items-center gap-2 rounded-lg px-2 py-2 text-left text-xs text-slate-600 hover:bg-blue-50 hover:text-primary">{item.logo_url ? <img src={item.logo_url} alt="" className="h-7 w-7 rounded object-contain"/> : <span className="grid h-7 w-7 place-items-center rounded bg-slate-100"><ImageIcon className="h-3.5 w-3.5 text-slate-400"/></span>}<span className="truncate">{item.name}</span></button>)}{category && !subcategories.length && <p className="p-3 text-xs text-slate-400">No subcategories for this type.</p>}</div></div></div>}</div>
}
