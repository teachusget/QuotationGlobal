...
export default function FeaturedAdsCarousel({ ads = [], services = [], preview = false, placement = 'hero' }) {
  ...
  return <article className={`featured-ads-carousel group relative mx-auto min-h-72 overflow-hidden rounded-3xl border border-white/25 bg-white text-slate-900 shadow-floating transition hover:-translate-y-0.5 hover:shadow-overlay ${placement === 'hero' ? 'max-w-md' : placement === 'footer' ? 'max-w-5xl' : 'max-w-3xl'}`}>
