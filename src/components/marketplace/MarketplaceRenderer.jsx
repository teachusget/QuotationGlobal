...
export default function MarketplaceRenderer({ document, media = {}, catalog: suppliedCatalog, preview = false }) {
  ...
  const adVisibility = { banner: document?.advertisements_visibility?.banner !== false, catalog: document?.advertisements_visibility?.catalog !== false, footer: document?.advertisements_visibility?.footer !== false }
  document = {
    ...document,
    advertisements: {
      center: adVisibility.catalog ? document?.advertisements?.center || [] : [],
      footer: adVisibility.footer ? document?.advertisements?.footer || [] : [],
    },
  }
  ...
  return <div className={`marketplace-renderer marketplace-container-${theme.container || 'wide'} grid ${spacingClass[theme.section_spacing] || spacingClass.comfortable}`} style={style}>{(document?.sections || []).map((block) => <Fragment key={block.id}>{renderBlock(block)}{block.type === 'hero' && adVisibility.banner && block.settings?.featured_ads?.length > 0 && <aside aria-label="Banner advertisements" className="rounded-3xl bg-gradient-to-r from-[#06285e] to-primary p-5"><FeaturedAdsCarousel placement="hero" preview={preview} ads={block.settings.featured_ads} services={catalog.services || []}/></aside>}</Fragment>)}{document?.advertisements?.footer?.length > 0 && <aside className="rounded-3xl bg-gradient-to-r from-slate-950 via-[#082d66] to-primary p-5 sm:p-7"><p className="mb-3 text-[10px] font-bold uppercase tracking-[.18em] text-blue-100">Sponsored partner</p><FeaturedAdsCarousel placement="footer" preview={preview} ads={document.advertisements.footer} services={catalog.services || []}/></aside>}</div>
}
