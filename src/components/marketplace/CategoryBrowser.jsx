...
export default function CategoryBrowser({ categories, selected, onSelect, loading = false }) {
  ...
  return <div className="mt-9 rounded-2xl border bg-white px-4 py-6 shadow-subtle sm:px-6">
    ...
    <div className="mt-6 grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8">
      {loading && !tiles.length ? Array.from({ length: 16 }, (_, index) => <div key={index} className="flex min-h-[92px] flex-col items-center justify-center rounded-lg border bg-white"><span className="h-9 w-9 animate-pulse rounded-lg bg-slate-100"/><span className="mt-2 h-2.5 w-16 animate-pulse rounded bg-slate-100"/></div>) : tiles.map((item, index) => {
        ...
        return <button
          ...
          className={`group flex min-h-[92px] flex-col items-center justify-center rounded-lg border px-2 py-3 text-center transition ${active ? 'border-primary bg-blue-50 ring-1 ring-primary' : 'bg-white hover:-translate-y-0.5 hover:border-blue-300 hover:shadow-md'}`}
        >
          {item.logo_url
          ...
          <span className="mt-2 line-clamp-2 text-[10px] font-semibold leading-4 text-slate-700">{item.name}</span>
        </button>
      })}
    </div>
