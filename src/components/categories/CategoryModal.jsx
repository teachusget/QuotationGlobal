...
export default function CategoryModal({ open, type, categories, item, onClose, onSave, saving, serverError, onClearError }) {
  ...
  useEffect(() => {
    if (!open) return
    const escape = (event) => event.key === 'Escape' && !saving && onClose()
  ...
  return <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
    ...
    <div role="dialog" aria-modal="true" aria-labelledby="category-modal-title" className="relative max-h-[92vh] w-full max-w-lg overflow-y-auto rounded-lg bg-white shadow-xl">
      <div className="flex h-14 items-center justify-between border-b px-5">
        <h2 id="category-modal-title" className="text-base font-bold">{title}</h2>
        <button onClick={onClose} disabled={saving} className="rounded-md p-2 text-slate-500 hover:bg-slate-100 disabled:opacity-50" aria-label="Close"><X className="h-4 w-4"/></button>
      </div>
      <form onSubmit={submit} className="p-5">
        ...
        <div className="mt-6 flex justify-end gap-2 border-t pt-4"><button type="button" onClick={onClose} disabled={saving} className="h-9 rounded-md border px-4 text-xs font-semibold text-slate-600 hover:bg-slate-50 disabled:opacity-50">Cancel</button><button disabled={saving} className="h-9 rounded-md bg-primary px-4 text-xs font-semibold text-white hover:bg-blue-700 disabled:cursor-wait disabled:opacity-60">{saving ? 'Saving...' : item ? 'Save changes' : `Add ${entityLabel.toLowerCase()}`}</button></div>
      </form>
    </div>
  </div>
}
