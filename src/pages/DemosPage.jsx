...
function ChatModal({ request, currentUser, onClose, onModerationChanged }) {
  ...
  const isAdmin = !['buyer', 'vendor'].includes(currentUser.role)
  const currentUserBlocked = currentUser.role === 'buyer' ? moderation.buyer_chat_blocked : currentUser.role === 'vendor' ? moderation.vendor_chat_blocked : false
  ...
  return <div className="fixed inset-0 z-[80] grid place-items-center p-4">
    ...
    <div role="dialog" aria-modal="true" aria-labelledby="demo-chat-title" className="relative flex h-[min(680px,90vh)] w-full max-w-2xl flex-col overflow-hidden rounded-xl bg-white shadow-2xl">
      <div className="flex items-center justify-between border-b px-5 py-4">
        <div><h2 id="demo-chat-title" className="font-bold">{request.service?.name} â€” Meeting Chat</h2><p className="mt-0.5 text-xs text-slate-500">{isAdmin ? `${request.user?.name} â†” ${request.vendor?.company_name || request.vendor?.name}` : new Date(request.demo_at).toLocaleString()}</p></div>
      ...
      <div className="min-h-0 flex-1 space-y-3 overflow-y-auto bg-slate-50 p-5">
        {visibleMessages.map((message) => {
          if (message.is_system) return <div key={message.id} className="mx-auto max-w-lg overflow-hidden rounded-xl border border-emerald-200 bg-white shadow-sm"><div className="flex items-center gap-2 bg-emerald-50 px-4 py-3 text-emerald-800"><CheckCircle2 className="h-5 w-5"/><div><p className="text-xs font-bold">Demo request accepted</p><p className="text-[10px] text-emerald-700">Your meeting has been confirmed by the Solution Provider.</p></div></div><div className="p-4"><div className="flex items-center gap-3 rounded-lg bg-slate-50 p-3"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-blue-100 text-primary"><CalendarClock className="h-4 w-4"/></span><div><p className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">Meeting date & time</p><p className="mt-0.5 text-xs font-bold text-slate-800">{meetingDate?.toLocaleDateString(undefined, { weekday: 'short', day: '2-digit', month: 'short', year: 'numeric' })} Â· {meetingDate?.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p></div></div>{request.meeting_link && <a href={request.meeting_link} target="_blank" rel="noreferrer" className="mt-3 flex h-9 w-full items-center justify-center gap-2 rounded-lg bg-primary text-xs font-semibold text-white hover:bg-blue-700"><Video className="h-4 w-4"/>Join Online Meeting <ExternalLink className="h-3.5 w-3.5"/></a>}</div></div>
          const mine = message.user_id === currentUser.id
          return <div key={message.id} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[78%] rounded-xl px-4 py-2.5 ${mine ? 'bg-primary text-white' : 'border bg-white text-slate-700'}`}><p className={`mb-1 text-[10px] font-semibold ${mine ? 'text-blue-100' : 'text-slate-400'}`}>{mine ? 'You' : `${message.user?.name || 'System'}${message.user?.role && !['buyer', 'vendor'].includes(message.user.role) ? ' (Admin)' : ''}`}</p>{message.attachment_type === 'image' && <button type="button" onClick={() => openImage(message)} className="group relative mb-2 block overflow-hidden rounded-lg bg-slate-900/10 text-left"><img src={message.attachment_data} alt={message.attachment_name || 'Chat attachment'} className="max-h-64 min-h-20 w-full object-contain"/><span className="absolute inset-0 grid place-items-center bg-slate-950/0 opacity-0 transition group-hover:bg-slate-950/35 group-hover:opacity-100"><span className="inline-flex items-center gap-1.5 rounded-full bg-white/95 px-3 py-1.5 text-[10px] font-semibold text-slate-800 shadow"><ZoomIn className="h-3.5 w-3.5"/>View image</span></span></button>}{message.attachment_type === 'audio' && <div className={`mb-2 rounded-lg p-2.5 ${mine ? 'bg-white/15' : 'bg-slate-50'}`}><div className={`mb-1.5 flex items-center gap-1.5 text-[10px] font-semibold ${mine ? 'text-blue-50' : 'text-slate-500'}`}><Mic className="h-3.5 w-3.5"/>Voice note</div><audio controls preload="metadata" className="h-9 w-[min(280px,65vw)] max-w-full" src={message.attachment_data}>Your browser does not support audio playback.</audio></div>} {message.message && <p className="whitespace-pre-wrap break-words text-xs leading-5">{message.message}</p>}<p className={`mt-1 text-right text-[9px] ${mine ? 'text-blue-100' : 'text-slate-400'}`}>{new Date(message.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p></div></div>
        })}
...
function AcceptModal({ request, saving, error, onClose, onAccept }) {
  ...
  return <div className="fixed inset-0 z-[80] grid place-items-center p-4">
    ...
    <form onSubmit={submit} className="relative w-full max-w-md rounded-xl bg-white p-5 shadow-2xl">
...
function RejectModal({ request, saving, error, onClose, onReject }) {
  ...
  return <div className="fixed inset-0 z-[80] grid place-items-center p-4">
    <button type="button" onClick={onClose} className="absolute inset-0 bg-slate-950/45" aria-label="Close"/>
    <form onSubmit={submit} className="relative w-full max-w-md rounded-xl bg-white p-5 shadow-2xl">
      <div className="flex items-start justify-between border-b pb-4"><div><h2 className="font-bold">Reject Demo Request</h2><p className="mt-1 text-xs text-slate-500">Customer ko rejection ki clear wajah batayein.</p></div><button type="button" onClick={onClose} className="rounded-md p-1 text-slate-500 hover:bg-slate-100"><X className="h-5 w-5"/></button></div>
      ...
      <div className="mt-1 flex justify-between text-[10px] text-slate-400"><span>Minimum 5 characters</span><span>{reason.length}/1000</span></div>
      {error && <p className="mt-3 rounded-md bg-red-50 p-2 text-xs text-red-600">{error}</p>}
      ...
      <table className="w-full min-w-[900px] text-left text-xs">
        ...
        <tbody>
          {requests.map((item) => <tr key={item.id} className="border-t">
            ...
            <td className="p-3"><span className={`rounded-full px-2 py-1 text-[10px] font-semibold ${item.status === 'accepted' ? 'bg-emerald-50 text-emerald-700' : item.status === 'rejected' ? 'bg-red-50 text-red-700' : 'bg-amber-50 text-amber-700'}`}>{item.status}</span>{item.status === 'rejected' && item.rejection_reason && <p className="mt-2 max-w-[230px] text-[10px] leading-4 text-red-600" title={item.rejection_reason}><span className="font-semibold">Reason:</span> {item.rejection_reason}</p>}</td>
            <td className="p-3">{item.status === 'accepted' ? <div className="flex items-center gap-2"><button type="button" onClick={() => { setChatRequest(item); setRequests((current) => current.map((row) => row.id === item.id ? { ...row, unread_count: 0 } : row)) }} className="relative inline-flex items-center gap-1.5 rounded-md border border-blue-200 px-2.5 py-1.5 text-[10px] font-semibold text-primary hover:bg-blue-50"><MessageCircle className="h-3.5 w-3.5"/>Chat{Number(item.unread_count) > 0 && <span className="absolute -right-2 -top-2 grid h-5 min-w-5 place-items-center rounded-full bg-red-500 px-1 text-[9px] font-bold text-white ring-2 ring-white">{item.unread_count > 9 ? '9+' : item.unread_count}</span>}</button>{item.meeting_link && <a href={item.meeting_link} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-[10px] font-semibold text-primary hover:underline"><ExternalLink className="h-3.5 w-3.5"/>Join Meeting</a>}</div> : <span className="text-slate-400">Available after acceptance</span>}</td>
            {isVendor && <td className="p-3">{item.status === 'pending' ? <div className="flex gap-2"><button disabled={Boolean(updating)} onClick={() => { setError(''); setAcceptRequest(item) }} className="rounded bg-emerald-600 px-2.5 py-1.5 text-[10px] font-semibold text-white disabled:opacity-50">Accept</button><button disabled={Boolean(updating)} onClick={() => updateStatus(item.id, 'rejected')} className="rounded border border-red-200 px-2.5 py-1.5 text-[10px] font-semibold text-red-600 disabled:opacity-50">Reject</button></div> : <span className="text-slate-400">â€”</span>}</td>}
          </tr>)}
