...
export default function LoginPage() {
  ...
  useEffect(() => { document.title = 'Admin Login | Quotation Global' }, [])
  ...
  return <main className="grid min-h-screen bg-white lg:grid-cols-[minmax(420px,46%)_1fr]">
    <section className="flex min-h-screen items-center justify-center px-5 py-10 sm:px-10">
      <div className="w-full max-w-[390px]">
        <BrandLogo />
        <div className="mt-12"><span className="inline-flex items-center gap-1.5 rounded-md bg-blue-50 px-2.5 py-1 text-[11px] font-semibold text-primary"><LockKeyhole className="h-3.5 w-3.5"/>Admin portal</span><h1 className="mt-4 text-2xl font-bold text-slate-900">Welcome back</h1><p className="mt-2 text-sm text-slate-500">Sign in to manage Quotation Global.</p></div>
        <form onSubmit={submit} className="mt-8 space-y-4" noValidate>
          ...
          <button disabled={submitting} className="flex h-11 w-full items-center justify-center rounded-md bg-primary text-sm font-semibold text-white transition hover:bg-blue-700 disabled:cursor-wait disabled:opacity-70">{submitting ? 'Signing in...' : 'Sign in'}</button>
        </form>
        ...
        <p className="mt-8 text-center text-[10px] text-slate-400">© 2026 Quotation Global. All rights reserved.</p>
      </div>
    </section>
