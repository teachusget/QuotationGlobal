...
export default function SpecificationTemplatesPage() {
  ...
  return <section>
    ...
    <form onSubmit={save} className="mt-5 rounded-lg border bg-white p-4">
      ...
      <div className="mt-5 rounded-lg border border-blue-100 bg-blue-50/50 p-4"><h2 className="text-xs font-bold">Specifications / feature points</h2><p className="mt-1 text-[10px] text-slate-500">Add all points one by one. The settings selected above apply to every point in this batch.</p><div className="mt-3 flex gap-2"><input value={point} onChange={(e) => setPoint(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); addPoint() } }} maxLength={120} placeholder="e.g. Role-Based Access Control" className="h-10 min-w-0 flex-1 rounded-md border bg-white px-3 text-sm"/><button type="button" onClick={addPoint} className="inline-flex h-10 items-center gap-1 rounded bg-primary px-4 text-xs font-semibold text-white"><Plus className="h-4 w-4"/>Add</button></div><div className="mt-3 flex flex-wrap gap-2">{points.map((item, index) => <span key={`${item}-${index}`} className="inline-flex items-center gap-1.5 rounded-full border bg-white px-3 py-1.5 text-xs font-medium">{item}<button type="button" onClick={() => setPoints((current) => current.filter((_, itemIndex) => itemIndex !== index))}><X className="h-3.5 w-3.5 text-red-500"/></button></span>)}{!points.length && <span className="text-[10px] text-slate-400">No specifications added yet.</span>}</div></div>
