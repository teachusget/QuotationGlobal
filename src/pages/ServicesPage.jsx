...
export default function ServicesPage() {
  ...
  const [services, setServices] = useState([]); const [categories, setCategories] = useState([]); const [subs, setSubs] = useState([]); const [industries, setIndustries] = useState([]); const [brands, setBrands] = useState([]); const [vendors, setVendors] = useState([]); const [form, setForm] = useState(blank); const [open, setOpen] = useState(false); const [editingId, setEditingId] = useState(null); const [error, setError] = useState(''); const [saving, setSaving] = useState(false); const [imageFiles, setImageFiles] = useState([])
  useEffect(() => {
    const load = async () => {
  ...
  useEffect(() => {
    const addImages = (event) => setImageFiles((current) => [...current, ...event.detail].slice(0, 8))
    ...
    const editProduct = (event) => {
      ...
      setEditingId(service.id); setError(''); setOpen(true)
...
function ServiceTable({ services, showVendor = false }) {
  ...
  return <div className="mt-6 overflow-x-auto rounded-lg border bg-white">
    <table className="w-full min-w-[940px] text-left text-xs">
      <thead className="bg-slate-50 text-[10px] uppercase text-slate-500"><tr><th className="p-3">Service</th>{showVendor && <th className="p-3">Vendor</th>}<th className="p-3">Type</th><th className="p-3">Industries</th><th className="p-3">Brand</th><th className="p-3">Pricing Plans</th><th className="p-3">Action</th></tr></thead>
