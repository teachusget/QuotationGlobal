...
const plans = { map: (callback) => (pricingServiceType === 'hardware' ? [] : pricingServiceType === 'services' ? servicePlans : subscriptionPlans).map(callback) }
  ...
  return <div className="mt-6 overflow-x-auto rounded-lg border bg-white">
    <table className={`service-data-table ${showVendor ? 'with-vendor' : ''} w-full min-w-[940px] text-left text-xs`}>
      ...
      <tbody>{grouped.map((service) => <tr key={service.id} className="border-t align-top">
        ...
        <td className="p-3"><div className="flex max-w-sm flex-wrap gap-1.5">{service.plans.map((plan) => <span key={plan.id} className="rounded bg-slate-100 px-2 py-1 leading-5"><b>{service.service_type === 'hardware' ? 'Unit Price' : planLabels[plan.billing_cycle]}</b> <strong className="ml-1 text-primary">{plan.pricing_mode === 'flexible_price' ? `Flexible $${Number(plan.price_from).toLocaleString()}` : `$${Number(plan.price_from).toLocaleString()}`}</strong>{Number(plan.discount_percent || 0) > 0 && <em className="ml-1 not-italic text-emerald-700">{plan.discount_percent}% OFF</em>}</span>)}</div></td>
...
function Text({ label, value, onChange, required, type = 'text', wide }) {
  ...
  const isHardwarePrice = serviceType === 'hardware' && (label === 'Fixed Monthly Price (USD)' || label === 'Flexible Starting Price (USD)')
  const shownLabel = isHardwarePrice
    ? (label === 'Flexible Starting Price (USD)' ? 'Flexible Starting Price (USD)' : 'Unit Price (USD)')
    : label === 'Service Name' && serviceType === 'hardware'
    ? 'Product Name'
    : serviceType === 'services' && type === 'number'
      ? (label.startsWith('Flexible') ? 'Flexible Monthly Base Rate (USD)' : 'Fixed Monthly Base Rate (USD)')
      : label
  return <div className={`text-xs font-semibold ${wide ? 'sm:col-span-2' : ''}`}>
    <label>{shownLabel}{required && ' *'}<input data-hardware-unit-price={isHardwarePrice || undefined} type={type} value={value} required={required} min={type === 'number' ? '0' : undefined} step={type === 'number' ? '0.01' : undefined} onChange={(e) => onChange(e.target.value)} className="mt-1.5 h-10 w-full rounded-md border px-3 text-sm font-normal"/></label>
